 ...
 */
async function constructServer(moduleDefs, options = {}) {
  const app = express()
  
  // Rewrite /api/* prefix to /* transparently for production compatibility
   ...
   */
  const MAX_UPLOAD_SIZE_MB = process.env.ICHIGO_DESKTOP === 'true' ? 10 : 500
   ...
   * Cache
   */
  if (process.env.ICHIGO_DESKTOP !== 'true') {
  ...
  // If staticPath is provided, register React Router catch-all fallback
  if (options.staticPath) {
    app.use((req, res, next) => {
...
async function serveNcmApi(options) {
  ...
  const constructServerSubmission = constructServer(options.moduleDefs, options)
