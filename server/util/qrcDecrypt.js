...
async function decompressDeflate(bytes) {
  ...
  const inflate = (fn) => new Promise((resolve, reject) => {
    fn(buffer, (err, out) => err ? reject(err) : resolve(out));
  });
  try {
    return (await inflate(zlib.inflate)).toString('utf8');
  } catch (err) {
    return (await inflate(zlib.inflateRaw)).toString('utf8');
  }
}
