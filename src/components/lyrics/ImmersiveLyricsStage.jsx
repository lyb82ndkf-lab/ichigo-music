...
const IMMERSIVE_RENDERERS = {
  ...
  talk: { component: TiltLyrics, props: ({ config }) => ({ themeColor: 'var(--primary)', showGlow: config?.showGlow === true, globalOffset: config?.globalOffset || 0 }) },
  ...
  spatial: { component: SpatialCanvasLyrics, props: ({ config, themeColor }) => ({ themeColor, globalOffset: config?.globalOffset || 0 }) },
  vinyl: { component: VinylRecordLyrics, props: ({ config, themeColor, coverUrl, isPlaying }) => ({ themeColor, coverUrl, isPlaying, globalOffset: config?.globalOffset || 0, lineSpacing: config?.vinylLineSpacing ?? 0.7, tiltAngle: config?.vinylTiltAngle ?? 0 }) },
...
export default function ImmersiveLyricsStage({ mode, lyrics = [], activeLineIndex = -1, engineRef, dimensions, fontStack, themeColor, coverUrl, isPlaying, config }) {
  ...
  const modeProps = renderer.props({ config, themeColor, coverUrl, isPlaying, fontPx: dimensions.fontPx, translationPx: dimensions.transPx });
