...
import { Play, Heart, Trash2, History, Calendar, ListMusic, Sparkles } from 'lucide-react';
import ListeningHeatmap from '../components/ListeningHeatmap';

export default function RecentlyPlayed() {
  const { recentlyPlayed, playSong, likedSongIds, toggleLike, navigateTo, viewData } = useApp();
  const [activeTab, setActiveTab] = useState(() => viewData?.tab === 'heatmap' ? 'heatmap' : 'list');

  useEffect(() => {
    if (viewData?.tab === 'heatmap' || viewData?.tab === 'calendar') {
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            borderRadius: '99px',
            border: 'none',
            background: activeTab === 'list' ? 'var(--primary)' : 'var(--glass-bg, rgba(255, 255, 255, 0.06))',
            color: activeTab === 'list' ? '#ffffff' : 'var(--text-muted)',
            fontSize: '13px',
            fontWeight: 700,
            cursor: 'pointer',
            boxShadow: activeTab === 'list' ? '0 4px 16px var(--primary-glow, rgba(255,64,129,0.35))' : 'none',
            transition: 'all 0.2s cubic-bezier(0.16, 1, 0.3, 1)'
          }}
        >
          <ListMusic size={16} />
          <span>播放记录列表 ({recentlyPlayed.length})</span>
        </button>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: '99px',
            border: 'none',
            background: activeTab === 'heatmap' ? 'var(--primary)' : 'var(--glass-bg, rgba(255, 255, 255, 0.06))',
            color: activeTab === 'heatmap' ? '#ffffff' : 'var(--text-muted)',
            fontSize: '13px',
            fontWeight: 700,
            cursor: 'pointer',
            boxShadow: activeTab === 'heatmap' ? '0 4px 16px var(--primary-glow, rgba(255,64,129,0.35))' : 'none',
            transition: 'all 0.2s cubic-bezier(0.16, 1, 0.3, 1)'
          }}
        >
          <Calendar size={16} />
      ...
      {activeTab === 'heatmap' ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: '28px', marginBottom: '32px', alignItems: 'center' }}>
            ...
            >
              <History size={48} color="#ffffff" style={{ filter: 'drop-shadow(0 4px 12px rgba(0,0,0,0.3))' }} />
            </div>
            <div>
                      ...
                      >
                        ...
                        <td>
                          <span style={{ color: 'var(--text-muted)' }}>
                            {formatDuration(song.dt || song.duration || 0)}
                          </span>
                        </td>
                      </tr>
                    );
